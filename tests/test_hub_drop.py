...
# ─── 콜드스타트 완주 관측표 (0.4.18, Ray 062 §2) ───────────────────────────
# 기본 예산(CLIENT_TIMEOUT_SECONDS)이 서 있는 근거의 원장. **완주만** 싣는다 —
# 상한에 잘린 값은 길이가 아니라 하한이므로 별도 목록(_COLDSTART_CLIPPED)에.
# 상한 병기 필수(Ray 계측 규칙: 상한 없는 값은 길이인지 절단인지 영원히 모른다).
# 새 완주 관측이 도착하면 **여기 행을 추가하라** — 최댓값이 자라면 아래 시험이
# 기본값을 함께 올리라고 시끄럽게 말한다. 그게 이 표가 존재하는 이유다.
_COLDSTART_COMPLETIONS = [
    # (완주 초, 상한 초, 잰 집, 자리)
    (102.1, 120, "lxm",     "무인증 GET, 한가한 시각 (08-27)"),
    (170.4, 400, "ray",     "T-5 밖, 수동 회차 (08-30)"),
    (207.2, 400, "ray",     "T-5 (08-30)"),
    (208.3, 400, "ray",     "T-5 (08-29)"),
    (211.6, 300, "organum", "앵커 밖 발신 push — 0.4.17 출하 통지 자신 (08-31)"),
    (229.2, 400, "ray",     "T-5 (08-30)"),
    (248.2, 400, "ray",     "T-5 (08-29)"),
    (120.2, 300, "organum", "D2 리허설 재푸시, 앵커 밖 (09-02)"),
    (292.8, 400, "ludex",   "수거기 19:05 KST — 첫 '표가 기본값을 밀어올린' 표본 (09-02, 089)"),
    (322.5, 350, "organum", "r3 청구 071 push, 앵커 밖 — 350×margin 안, 완주 최대 경신 (09-02)"),
    (352.6, 400, "ludex",   "수거기 22:02 KST — 새 기본 350을 하루 만에 넘긴 표본 (09-03, 093)"),
    (352.6, 420, "organum", "channels 조회 GET, 앵커 밖 — 0.5.1 후보의 자기 실측 (09-11)"),
]
_COLDSTART_CLIPPED = [
    # 하한만 말하는 값들 — 최댓값 계산에 못 들어간다
    (150.1, 150, "lxm",   "수거 pull 워밍, 자기 예산에 잘림 (08-30)"),
    (120.0, 120, "ray",   "push budget 120에 절단, Win/0.4.17 클라 (09-02, 073)"),
    (400.1, 400, "ludex", "수거기 18:39 KST — 계기 천장 400조차 분포 안이라는 신호 (09-02, 089)"),
]
# 표본 한계(Ray 062 §4, 상수 곁에 병기): 완주 12 중 4가 한 호스트(Ray Win11)·한
# 문(from-ray)·T-5 클러스터. ≥400 잘린 값(09-02)은 **계기 천장 400조차 분포
# 안일 수 있다**는 신호다 — 분포 위끝은 여전히 미지, 계기 천장 상향은 각 집 몫.
# 09-04부터 hosted drop 운용자(LxM 063)가 앵커 T-10 워밍·노크 상한 600으로 돌리므로
# 앵커 시각 수거는 이후 더운 호스트를 만나기 쉽다 — 앵커 밖 발신(우리 071·D2 재푸시
# 부류)이 콜드스타트 분포의 주 표본원으로 남는다. 표본의 시각 자리를 계속 병기하라.


def test_클라이언트_timeout_인자_배선(drop, tmp_path):
    """0.4.2: push/pull이 timeout을 받는다. 기본값의 성질(0.4.17→0.4.18 정련):
    **기본 예산은 관측된 완주 분포를 덮는다** — 근거는 위 관측표다.

    0.4.17의 첫 판(`> 248` 상수 박기)은 Ray 062가 지적한 대로 **한 방향만**
    지켰다: 내리면 빨간불인데, 완주 320이 관측돼도 초록 — "천장이 분포 안에
    들어왔는데 계기는 초록". 그래서 상수 대신 관측표의 최댓값을 참조한다 —
    이제 올라오는 관측이 도착할 곳(표)이 코드에 있다."""
    url, token, root = drop
    observed_max = max(v for v, _cap, _lab, _site in _COLDSTART_COMPLETIONS)
    assert hd.CLIENT_TIMEOUT_SECONDS > observed_max, (
        f"기본 예산({hd.CLIENT_TIMEOUT_SECONDS}s)이 관측표의 완주 최대"
        f"({observed_max}s) 아래다. 이 수는 _COLDSTART_COMPLETIONS의 최댓값이다 — "
        "표에 더 큰 완주가 들어왔으면 이 기본값을 함께 올리고, 내리려면 표를 "
        "반박하는 새 분포가 있어야 한다")
    for v, cap, lab, _site in _COLDSTART_COMPLETIONS:
        assert v < cap, (
            f"{lab}의 {v}s는 상한 {cap}s에 잘린 값이다 — 완주가 아니므로 "
            "_COLDSTART_CLIPPED로 옮겨라(잘린 값은 길이가 아니라 하한이다)")
    sdir = root / "ch" / "from-a"
    sdir.mkdir(parents=True)
    (sdir / "001-sig.txt").write_text("ab" * 64 + "\n")
