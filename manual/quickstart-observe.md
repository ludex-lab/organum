...
터미널 말고 브라우저로 보고 싶거나 누군가에게 공유하고 싶으면 `--html`:

```bash
organum-inspector . --html report.html          # 사후 계측 리포트 파일
organum observatory report --html report.html   # 지금/역사 밴드 리포트 파일
```

서버 없이 열리는 자립형 HTML 한 장이 떨어져요 — 타임라인, 세션 표, 벤더
비교 막대까지. 파일이니까 팀 채널에 던지거나 기록으로 보관하면 됩니다.
