...
`c%` 컬럼은 `cache/(in+cache)`, 즉 입력 중 캐시로 회수된 비율이에요. 하네스·
모델 조합에 따라 캐시 보존이 크게 달라서(우리 프로젝트 실측: 100% vs 49%),
API 비용을 좌우하는 첫 번째 레버입니다. 원천이 없는 벤더는
그대로 `—`.

### 비용 근사 — 단가는 당신이 채웁니다

organum은 단가표를 배포하지 않아요(가격은 바뀌고, 낡은 단가로 비용을 지어내는
것보다 비우는 게 정직해서). `~/.organum/prices.json`에 모델별 $/Mtok을 넣으면
벤더 합계에 `$` 근사가 나타나요 (API 단가 기준·구독은 제외, `--prices <파일>`로 지정도 가능):

```json
{ "solar-open2": { "in": 0.5, "out": 1.5, "cache_read": 0.05 } }
```

### role 주석과 reported 밴드 (organum 쓰는 프로젝트라면)

대상 폴더에 `.organum/`이 있으면 두 가지가 더 나타나요 — ① 세션이 선언한
역할(role)이 행에 주석되고 ② `organum observatory ingest`로 수용된 supervisor
하네스 run이 **별도 reported 밴드**로 나옵니다(passive 계측과 절대 합산하지
않아요 — supervised run은 격리 런타임이라 passive 계측에 안 보이기 때문에,
두 증거를 나란히 두는 게 정직한 전부입니다). 없으면 지금까지처럼 그대로 —
설정 없음 명제는 불변이에요.
