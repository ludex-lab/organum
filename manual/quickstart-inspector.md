...
`c%` 컬럼은 `cache/(in+cache)`, 즉 입력 중 캐시로 회수된 비율이에요. 같은
모델이라도 하네스에 따라 캐시 보존이 크게 달라서(우리 실측: 한 하네스 100% vs
다른 하네스 49%), API 비용을 좌우하는 첫 번째 레버입니다. 원천이 없는 벤더는
그대로 `—`.
...
벤더 합계에 `$` 근사가 나타나요 (`--prices <파일>`로 지정도 가능):
