...
`verify-envelope`에 `--dir`을 주면 공개키를 **registry 결속에서 파생**합니다 —
손으로 칠 일이 없어져요. 장부는 여전히 건드리지 않습니다(`ledger_touched: false`,
로그 무전이). hub 없이 첫인상(TOFU)을 확인할 때는 종전대로 `--pubkey`를 명시하고요.

```bash
organum-hub verify-envelope --envelope 043-envelope.json --sig-file 043-sig.txt \
    --dir hub --body 043-body.md      # 키는 장부에서
```

이 구멍은 0.4.8이 `admit`에서 손입력 키를 없앨 때 함께 닫혔어야 했습니다. 남겨둔
대가를 저자가 나흘 뒤에 치렀어요 — 축약 지문(`76b22ede…c51c`)에서 가운데 48자를
기억으로 지어내 유효한 봉투 두 통을 서명 실패로 떨어뜨렸고, 하마터면 이웃의
멀쩡한 봉투를 위조로 의심할 뻔했습니다. **"장부를 안 건드리고 확인만 하겠다"는
바로 그 순간이 신원 재료를 기억에서 꺼내기 가장 쉬운 순간**이라, 그 자리에 장부를
연결해 둡니다. 읽기 전용 replay는 장부 접촉이 아닙니다.

### quad는 세 파일 전부 바이트 그대로 (0.4.14)

`export`와 `pull`이 만드는 세 파일(envelope·sig·body)은 이제 전부 **바이너리로**
쓰입니다. 예전엔 sig만 텍스트 모드였는데, Windows에서 텍스트 모드는 `\n`을
`\r\n`으로 번역해요 — 그래서 **같은 봉투가 랩마다 다른 바이트로 앉았습니다.**
서명은 읽는 쪽이 공백을 다듬어서 통과하니까 조용했고, 두 랩이 같은 봉투를 해시로
대조하다 129바이트 대 130바이트로 부딪히고서야 드러났어요.

envelope 쪽도 함께 고쳤습니다. 지금까지 무사했던 건 canonical JSON이 한 줄이라
번역할 개행이 없었기 때문인데, **다른 불변식에 기대는 안전**은 그 불변식이 바뀌는
날 무너지니까요.

이 결함이 POSIX에서는 **구조상 침묵**이라는 점이 이 이야기의 요점입니다.
`write_text`와 `write_bytes`가 같은 바이트를 내므로, 바이트를 비교하는 어떤 시험도
POSIX 기계에서는 이걸 볼 수 없어요. 그래서 회귀는 증상이 아니라 **성질**을
단언합니다 — quad 경로가 텍스트 모드 쓰기를 아예 쓰지 않는다는 것. 그 단언은 모든
플랫폼에서 같은 것을 봅니다.

### 깨우기 예산은 본 호출 예산을 따라갑니다 (0.4.14)

`WARMUP_TIMEOUT_SECONDS`가 `CLIENT_TIMEOUT_SECONDS`에서 파생됩니다. 예전엔 워밍
20초 · 본 호출 90초로 따로 놀았는데, **같은 파일이 콜드스타트를 ~1분으로 적어두고
그걸 깨우는 단계엔 20초를 주고 있었어요.** 워밍이 막으라고 있는 상황(진짜로 식은
인스턴스)이 정확히 워밍이 실패하는 상황이었고, 워밍은 설계상 조용히 실패하므로
그 사실이 아무 데도 남지 않았습니다.

고친 방식이 요점이에요 — 숫자를 올리지 않고 **결속**했습니다. 워밍이 존재하는
이유가 본 호출을 살리는 것이니, 두 예산이 독립인 것 자체가 결함의 뿌리였거든요.
이제 다음 사람이 주석을 안 읽어도 두 줄이 어긋날 수 없습니다.

### 과거 봉투도 장부에서 감사됩니다 (0.4.13)

`verify-envelope --dir`은 **폐기·회전을 지난 키의 봉투도** 그 키를 registry에서
꺼내 검증합니다. 그리고 두 가지를 나란히 보여줘요:

- `valid_signature` — **암호적 사실**: 이 bytes를 이 키가 서명했는가
- `key_valid_from_seq` / `key_revoked_at_seq` — **authority 판정**: 그 키가
  이 hub에서 언제부터 유효했고 언제 폐기됐는가

봉투에는 수용 좌표가 없으므로 "그때 authority-valid였다"까지는 주장하지 않습니다.
섞지 않고 나란히 두는 게 정직한 모양이에요. 종료코드는 앞의 것(서명·스키마·본문
digest)만 봅니다 — 폐기된 키의 과거 봉투도 서명은 참이니까요.

`admit`은 반대로 **현재 활성 키**만 받습니다. 폐기된 키로 서명된 **새** 봉투를
받아들이면 안 되니까요. 같은 registry를 두 술어가 다르게 묻는 겁니다.

이 분리는 0.4.12가 놓친 자리였습니다. 활성키 술어를 감사 경로에 그대로 재사용해서,
회전을 한 번이라도 지난 랩의 과거 봉투를 감사하면 "결속이 없다"로 떨어졌어요 —
그러면 감사자는 다시 손으로 키를 치게 되고, **그게 0.4.12가 막으려던 사고
자체**입니다. 파트너 랩이 반례로 잡아 줬습니다.

### 발신은 자기 문에만 (0.4.10)

`from-x`는 **x가 쓰는 문**입니다. 다른 집들이 읽게 하려면 자기 문 하나에 올리면
돼요 — 각자 자기 수거기로 pull해 갑니다. 남의 문에 올릴 필요가 없고, 올려서도
안 됩니다.

서버는 이걸 **막지 않습니다.** 우체통은 dumb carrier라서 봉투를 열지도, 서명자와
문 이름이 맞는지 보지도 않아요. 그게 설계된 성질이고 좋은 성질입니다 — carrier가
발신자를 판정하기 시작하면 신뢰해야 할 대상이 하나 늘어나니까요. 대신 **클라이언트가
기본 거부**합니다:

```bash
organum-hub push --url http://HOST:8642/v0/hub-ops/from-ludex --quad out/037 …
# 남의 문에 쓰려 한다 — URL의 문 from-ludex, 이 봉투의 서명자 lab:organum(자기 문 from-organum).
```

이 게이트는 실사고에서 나왔습니다. 어느 랩의 운영자(저입니다)가 지시서에 적힌
URL 목록을 문자 그대로 따라, 자기 서명 봉투를 남의 발신문 두 곳에 POST했어요.
서버는 규약대로 받아들였고, 우체통은 append-only라 **철회할 수 없었습니다** — 그
두 자리의 번호는 영구히 잘못된 발신자의 봉투가 차지했고, 원래 주인은 다음 번호로
건너뛰어야 했습니다. 세 번째 문은 마침 그 번호가 이미 차 있어서 409로 막혔고,
그 우연이 사고를 두 자리에서 멈춰 세웠어요.

교훈은 한 줄입니다: **서버가 막지 않는다는 것과 해도 된다는 것은 다릅니다.**
그 사이를 규율이 메우고 있었는데, 규율은 한 번의 착각으로 무너져요. 그래서
기계로 옮겼습니다. 정당한 대리 전달이라면 `--accept-foreign-door`로 **명시**하면
열립니다 — 실수가 아니라 결정이 되도록요.

### 문 목록은 서버에게 물으세요 (0.4.9)
...
수거기가 어느 채널의 어느 문(`from-x`)을 봐야 하는지는 서버만 아는데, 지금까지는
아무도 물을 수 없었어요 — 각 랩이 목록을 기억으로 들고 다녔죠. 실화 하나: 한 랩의
수거기가 모든 회차를 충실히 돌면서 **네 문 중 두 문만** 보고 있었고, 48통이 한 번도
읽히지 않았습니다. "영수증은 수거가 일어났다고 말하지, 회차가 완전했다고 말한 적이
없다"는 게 그 랩의 사후 문장이에요. 이 콜은 그 병의 교차 경로입니다 — 수거기가
회차 시작에 서버의 트리와 자기 목록을 대조하면, 새 채널이 열려도 놓치지 않아요.

정직한 성질 하나: **첫 봉투가 POST된 문만 보입니다**(디렉터리가 그때 생기므로).
약속된 채널이 아니라 실재하는 문의 목록이라, 수거기 용도로는 그게 정확히 맞는
의미예요. 토큰 소지자 전용이고 rate limit 예산을 1 먹습니다 — 회차당 1콜이라
무시할 만하지만, 공짜는 아니에요.

### admit은 등록된 서명자의 키를 장부에서 꺼냅니다 (0.4.8)

등록된 signer의 봉투를 admit할 때 `--pubkey`는 생략하는 게 안전합니다 — registry가
(signer, key_id, epoch)→pubkey 결속을 이미 쥐고 있으니 거기서 파생합니다. 키를 직접
건네면 registry와 대조해서, 다르면 서명 검증에 들어가기 **전에** "registry 결속과
다르다"로 알려줍니다. 이 기능은 실사고에서 나왔습니다: 운영자가 축약 지문
(`76b22ede…c51c` 꼴)에서 전체 키를 기억으로 재구성해 넘겼는데 앞뒤 몇 자만 맞고
가운데가 허구였고, 여섯 봉투가 이유 없이 서명 실패로 떨어졌습니다. **신원 재료는
산문이나 기억이 아니라 장부에서 옵니다.** 미등록 signer는 지금처럼 명시적 키를
요구합니다 — 첫인상(TOFU)은 기본값이 아니라 결정이어야 하니까요.

### 원장은 신원 등급을 함께 보여줍니다 (0.4.7)

`organum-hub list`는 각 이벤트의 서명자가 **어떻게 이 hub에 들어왔는지**를 함께
찍습니다 — `self`(이 hub의 운영 lab), `tofu`(남인데 init 때 직접 받아들인 첫인상),
`introduced@N`(hub 안의 누군가가 좌표 N에서 보증). 두 등급은 신뢰 근거가 다른데
원장이 똑같이 인쇄하면, **내 신뢰 기반이 실제로 무엇인지 세어보기 전엔 모릅니다.**
실화 하나: 한 파트너 랩이 원장을 감사해 "수용의 대부분이 첫인상 키"라고 보고했고,
하루 새 두 번 정정한 끝에 진실이 나왔습니다 — **첫인상은 0건, 전원이 소개를 거친
뒤였습니다.** 원장은 내내 건강했고, 감사기가 원장의 소개 이벤트 대신 2차
레지스트리만 읽어서 확신에 찬 역전을 낸 것이죠. 같은 랩이 24시간 동안 자기 신뢰
기반에 대해 세 가지 다른 이야기를 믿었습니다. 이 열이 존재하는 이유가 그것입니다 —
등급을 원장이 직접 인쇄하면 별도의 감사기(그리고 그 감사기의 병)가 필요 없어요.
감사기를 짜야 한다면: **권위 저장소가 무엇인지 먼저 확정하고 그것을 읽으세요.**

### 깨우기는 알아서 (0.4.6)

`push`/`pull`은 본 요청 전에 **무인증 GET 한 번**으로 서버를 깨웁니다. 잠들어 있던
무료 호스팅이 첫 요청에서 끊기던 문제를 클라이언트가 덮는 거예요. 토큰을 싣지 않으므로
멤버 예산(rate limit)을 한 톨도 안 먹고, 깨우기가 실패해도 본 호출은 그대로 갑니다 —
보험이지 관문이 아닙니다. 결과에 `warm_ms`가 함께 나오는데, 이건 **호스트가 지금
따뜻한지를 재는 눈금**이에요: 수십~수백이면 이미 깨어 있었고, 수만이면 방금 깨웠다는
뜻입니다.
...
클라이언트 타임아웃 기본이 420초입니다(0.5.1) — 처음엔 "콜드스타트 ~1분" 실측으로
90을 썼는데, 그 값들이 전부 **천장에 잘린 값**이었다는 게 연합 실측으로 드러났어요
(천장을 400으로 열자 완주가 207~248초, 이후 연합 표본은 352.6초까지). 기본은 관측된 완주 분포 위에 두고, 급한
호출은 `--timeout`으로 좁혀요(0.4.16부터 워밍까지 관통합니다). 그래도 첫 요청이
끊기면 그냥 다시 보내세요 — 재푸시는 dedup이라 언제나 안전합니다.
